package org.magnum.symptoms.service.auth;
...
public class OAuth2SecurityConfiguration {
	...
	protected static class ResourceServer extends
		...
		public void configure(HttpSecurity http) throws Exception {
			 ...
			 * 
			 * // Require all other requests to have "write" scope http
			 * .authorizeRequests() .antMatchers("/**")
			 * .access("#oauth2.hasScope('write')");
			 */
			http.authorizeRequests().antMatchers(HttpMethod.GET, "/patient/**")
					.access("#oauth2.hasScope('patient_read')");
			http.authorizeRequests().antMatchers("/patient/**")
					.access("#oauth2.hasScope('patient_write')");
			http.authorizeRequests().antMatchers(HttpMethod.GET, "/doctor/**")
					.access("#oauth2.hasScope('doc_read')");
			http.authorizeRequests().antMatchers("/doctor/**")
					.access("#oauth2.hasScope('doc_write')");
	...
	protected static class OAuth2Config extends
		...
		public OAuth2Config() throws Exception {
			...
			/*
			 * ClientDetailsService csvc = new
			 * InMemoryClientDetailsServiceBuilder() // Create a client that has
			 * "read" and "write" access to the // video service
			 * .withClient("userAndroid").authorizedGrantTypes("password")
			 * .authorities("ROLE_CLIENT", "ROLE_TRUSTED_CLIENT")
			 * .scopes("read","write").resourceIds("video") .and() // Create a
			 * second client that only has "read" access to the // video service
			...
			ClientDetailsService csvc = new InMemoryClientDetailsServiceBuilder()
					...
					.scopes("patient_read", "patient_write")
					.resourceIds("video")
					.and()
					...
					.scopes("patient_read", "patient_write", "doc_read",
							"doc_write").resourceIds("video")
					.accessTokenValiditySeconds(3600).and().build();

			// Create a series of hard-coded users.
			/*
			 * UserDetailsService svc = new InMemoryUserDetailsManager(
			 * Arrays.asList( User.create("admin", "pass", "ADMIN", "USER"),
			 * User.create("user0", "pass", "USER"), User.create("user1",
			 * "pass", "USER"), User.create("user2", "pass", "USER"),
			 * User.create("user3", "pass", "USER"), User.create("user4",
			 * "pass", "USER"), User.create("user5", "pass", "USER")));
			 */
			UserDetailsService svc = new InMemoryUserDetailsManager(
					Arrays.asList(
							User.create("admin", "pass", "ADMIN", "USER"),
							User.create("patient01", "pat01", "ROLE_PATIENT"),
							User.create("patient02", "pat02", "ROLE_PATIENT"),
							User.create("patient03", "pat03", "ROLE_PATIENT"),
							User.create("patient04", "pat04", "ROLE_PATIENT"),
							User.create("patient05", "pat05", "ROLE_PATIENT"),
							User.create("doctor01", "doc01", "ROLE_DOCTOR"),
							User.create("doctor02", "doc02", "ROLE_DOCTOR")));
