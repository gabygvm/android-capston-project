...
public class OAuth2SecurityConfiguration {
	...
	protected static class OAuth2Config extends
		...
		@Autowired
		private UserDetailsService _userDetailsSvc;
...
*/
			ClientDetailsService csvc = new InMemoryClientDetailsServiceBuilder()
				...
				.scopes("patient_read","patient_write").resourceIds("video")
				.and()
				// Create a second client that only has "read" access to the
				// video service
				.withClient("userAndroid").authorizedGrantTypes("password")
				.authorities("ROLE_DOCTOR")
				.scopes("patient_read","patient_write","doc_read", "doc_write").resourceIds("video")
				.accessTokenValiditySeconds(3600).and().build();
...
*/
			/*UserDetailsService svc = new InMemoryUserDetailsManager(
					Arrays.asList(
							User.create("admin", "pass", "ADMIN", "USER"),
							User.create("pat00", "pat0", "ROLE_PATIENT"),
							User.create("pat01", "pat1", "ROLE_PATIENT"),
							User.create("pat02", "pat2", "ROLE_PATIENT"),
							User.create("pat03", "pat3", "ROLE_PATIENT"),
							User.create("doc00", "doc0", "ROLE_DOCTOR"),
							User.create("doc01", "doc1", "ROLE_DOCTOR")));
			...
			*/
			...
			combinedService_ = new ClientAndUserDetailsService(csvc, _userDetailsSvc);
		}
